...
def load_linear_regression_model():
    ...
    try:
        ...
        return None
# @st.cache_resource(ttl=3600, max_entries=1000)
# def load_random_forest_model():
#     """Load the random forest model"""
#     try:
#         model = joblib.load('models/new/gradient_boosting_pipeline.pkl')
#         return model
#     except Exception as e:
#         st.error(f"Could not load model: {e}")
#         return None
...
def handle_missing_values(df, reference_df):
    ...
    categorical_defaults = {
        ...
        'teacher_qualification': 'Intermediate',
    }
...
def validate_data_ranges(df, reference_df):
    ...
    ranges = {
        ...
        'PREV_GPA': (1.0, 5.0),
        'SAT': (1, 1500),
        'STUDENT_TEACHER_RATIO': (0.1, 100),
        'number_of_books': (0, 2000),
        'studying_hours': (40, 168),
    }
    ...
    def export_to_csv(self, df):
        ...
        column_order = [
            'timestamp', 'prev_sat', 'prev_gpa',
             'sat', 'student_teacher_ratio', 'studying_hours', 'number_of_books',
...
def make_prediction_with_history(model, input_data):
    ...
    model_rmse = 0.1174  # Replace with actual model RMSE
...
""", unsafe_allow_html=True)
    ...
    with col1: 
        ...
        studying_hours = st.number_input(
            "Students Studying Hours(Per Week)*",
            min_value=40,
            max_value=168,
            value=40,
            help="hours per week."
        ...
        number_of_books = st.number_input(
            "Number of Books Available (extbooks or resources)*",
            min_value=0,
            max_value=2000,
            value=100,
...
#         Basic: Represents lower qualifications (e.g., minimal training or experience).
# Intermediate: Represents moderate qualifications (e.g., standard teaching credentials).
# Advanced:
        descriptions = f""" 
    ...
    if st.button("Generate GPA Prediction", use_container_width=True):
        with st.spinner("Analyzing data and generating prediction..."):
            ...
            # model_rmse = 0.1935  # Approximate RMSE for confidence interval
            # margin = 1.96 * model_rmse  # 95% confidence interval
            # lower_bound = max(0.0, prediction - margin)
            # upper_bound = min(4.5, prediction + margin)
            model_rmse = 0.1935  # More realistic for correlation ~0.5-0.6
            margin = 1.96 * model_rmse  # 95% confidence interval
            lower_bound = max(0.0, prediction - margin)
            upper_bound = min(5.0, prediction + margin)  
